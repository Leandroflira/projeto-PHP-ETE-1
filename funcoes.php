...
function reduzirStr($str,$quantidade){
  ...
  function cadastrar($nome,$email,$peso,$altura,$imc,$classificacao)
    ...
    function registrarnoticia($titulo,$descricao,$imagem)
    {
        if(!$titulo || !$descricao || !$imagem){return;}

        $sql = "INSERT INTO `registrarnoticia` (`titulo`,`descricao`,`imagem`)
    ...
    {
        $pdo = Database::conexao();
        $sql = "SELECT * FROM registrarnoticia";
        $stmt = $pdo->prepare($sql);
        $list = $stmt->execute();
        $list = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $list;
    ...
    function buscarlogin($login)
    {
        ...
        $sql = "SELECT * FROM registrarnoticia WHERE login = '$login'";
    ...
    {
        if(!$categoria){return;}
        $sql = "INSERT INTO `categoria_tb` (`categoria`,`data`)
        VALUES(:categoria,:data)";
  ...
  {
      $pdo = Database::conexao();
      $sql = "SELECT * FROM `categoria_tb` WHERE `categoria` = '$termo'";
      $stmt = $pdo->prepare($sql);
      $list = $stmt->execute();
      $list = $stmt->fetchAll(PDO::FETCH_ASSOC);
